...
import { getItemWeightFormat } from "pcComponents/settings/itemSettings/action";
...
class ItemWeightInsertDialog extends Component {
    constructor (props) {
        super(props);
        this.state = {
            itemList :[],
    ...
    render () {
        ...
        return (
            <AtModal isOpened={isOpened} className='item-settings-page item-settings-dialog item-weight-insert-dialog' onClose={this.onClose}>
                    ...
                    <View className='my-table grid-item24'>
                        ...
                        <ScrollView scrollY style={{ height:'380px' }}>
                            <View className='body'>
                                {
                                    itemList.map(item =>
                                        <View className={`row grid-cont ${itemWeightMap[item.num_iid] ? 'row-disable' : ''}`}>
                                            ...
                                            <View className={`cell grid-item${item.grids[1]}`}>
                                                {
                                                    itemWeightMap[item.num_iid] ?
                                                        <Text>{getItemWeightFormat(item)}</Text>
                                                        :
...
export default ItemWeightInsertDialog;
